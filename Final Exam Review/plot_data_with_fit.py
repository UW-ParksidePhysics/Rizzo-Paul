...
def plot_data_with_fit(data, fit_curve, data_format=0, fit_format=0):
    ...
    fit_curve_x = list(un_fit_curve)

    a_x2, a_y2 = np.linspace(min(data), max(data)), np.array(fit_curve[1])
    if data_format == 1:
        plt.plot(a_x1, a_y1, 'ro')
    elif data_format == 2:
        plt.plot(a_x1, a_y1, 'bo')
    elif data_format == 3:
        plt.plot(a_x1, a_y1, 'yo')
    else:
